...
> **Directive adaptation (this repo).** The spec below describes the regulation/OLP shape used across the
> family. CCD2 is an **adopted directive**, so this repo's `STATUS.md` deviates as follows (see
> [`CLAUDE.md`](../CLAUDE.md) → *Directive adaptation*):
> - The dashboard rows are **Instrument · Adopted/in force · Legal base · Transposition deadline ·
>   Application date · EU enforcement · NL status · As of** (not Procedure/Proposal/OLP stage/Latest text).
> - "Where things stand" uses **two blocks** — *(A) EU instrument* (the directive + Commission-as-enforcer)
>   and *(B) Netherlands transposition* (Regering/MinFin, Tweede Kamer, Eerste Kamer, the AMvB, AFM) — each
>   keeping the same five fields (Stage · Latest act · Owner · Position · Next).
> - The "What changed" lanes are **current NL/CCD1 law → CCD2 directive → Dutch implementation (36924/AMvB)**;
>   the Commission/Council extract links become [`extracts/directive/`](../extracts/directive/) and
>   [`extracts/transposition-nl/`](../extracts/transposition-nl/).
>
> The generic spec below is retained as the family reference and the backport baseline.

## Fixed section order

1. `# Status snapshot — Directive (EU) 2023/2225 (CCD2)` + the **dashboard table** + the not-legal-advice blockquote
2. `## One-line status`
...
analysis page, and end each substantive cell with a deep link into the operative text — the baseline column
into [`extracts/directive/`](../extracts/directive/), the implementation column into the matching
[`extracts/transposition-nl/`](../extracts/transposition-nl/) `#anchor` (prefer a stable explicit `<a id=…>`
anchor over a heading slug).
...
| New transposition stage (nota van wijziging / enacted wet / AMvB) | dashboard **NL status** + the NL actor's **Latest act**; refresh the **What changed** headline table | the full [`docs/what-changed.md`](what-changed.md) table, [`positions.csv`](../data/positions.csv), [`extracts/transposition-nl/`](../extracts/transposition-nl/) |
